// 초기 상태 설정
const initState = {
  mbtiResult: '', //결과값이들어가는 mbtiresult
  page: 0, //0페이지는 인덱스
  survey: [
    {
      //객체를 가지고잇는데 퀘스쳔키를 가진 ..
...
// initState.mbtiResult
// initState['test'] //위아래가 같은거임(?)
...
//페이지만 넘기면되니까 인자를 받을필요없음 NEXT랑 RESET은. 타입은 필수라 타입만 전달하면됨
//CHECK는 뭔가 결과를 전달해야됨
...
export function check(result) {
  return {
    type: CHECK,
    payload: { result }, //페이로드는 객체안에 원하는 값이 들어있다
  };
}
...
export default function mbti(state = initState, action) {
  switch (action.type) {
    case CHECK:
      return {
        ...state,
        mbtiResult: state.mbtiResult + action.payload.result, //만약에 저기 payload대신 result라고썻으면 여기에는 action.result라고만쓰면됨 (?)
      };
    case NEXT:
      return {
        ...state,
